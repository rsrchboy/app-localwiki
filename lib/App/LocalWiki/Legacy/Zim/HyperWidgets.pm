package App::LocalWiki::Legacy::Zim::HyperWidgets;
...
sub _build_widget {
    ...
    my $text = widget '+Gtk2::Ex::HyperWYSIWYG' => (
        signal_connect => {
    ...
    # XXX
    Gtk2::Ex::HyperTextBuffer::create_default_tags($text->get_buffer);
...
sub on_key_press_event { # some extra keybindings
    ...
    if ($val == $k_return or $val == $k_kp_enter) { # Enter
        ...
        $self->parse_line($iter, $wysiwyg) or return 0;
